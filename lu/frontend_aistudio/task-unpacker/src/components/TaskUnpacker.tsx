...
import { FileText, Mail, Phone, Calendar, CheckCircle, Box, ArrowRight, Sparkles, Archive, Edit, Trash2, Clock, Check, List, ChevronLeft, ChevronRight, Minimize2, Maximize2, PackageOpen, X, Network } from 'lucide-react';
...
interface Task {
  ...
  progress: number;
  delay: number; // Simulation delay
  duration: number; // Processing duration
  details?: string; // Extra info for the card stack
}

const MOCK_TASKS: Task[] = [
  { id: '1', type: 'email', title: 'Send meeting minutes', recipient: 'Team', status: 'pending', progress: 0, delay: 500, duration: 2000, details: 'Draft minutes from the strategy session and distribute to all attendees. Include action items.' },
  { id: '2', type: 'calendar', title: 'Schedule follow-up', recipient: 'Client A', status: 'pending', progress: 0, delay: 1200, duration: 1500, details: 'Find a 30-min slot next Tuesday for the quarterly review sync.' },
  { id: '3', type: 'call', title: 'Clarify requirements', recipient: 'Product Owner', status: 'pending', progress: 0, delay: 800, duration: 3000, details: 'Discuss the edge cases for the new authentication flow.' },
  { id: '4', type: 'email', title: 'Intro to Design Team', recipient: 'Sarah', status: 'pending', progress: 0, delay: 2500, duration: 1800, details: 'Connect Sarah with the lead designer to start the asset handover.' },
  { id: '5', type: 'research', title: 'Competitor Analysis', recipient: 'Internal', status: 'pending', progress: 0, delay: 200, duration: 4000, details: 'Review the top 3 competitors pricing models and feature sets.' },
  { id: '6', type: 'calendar', title: 'Block focus time', recipient: 'Self', status: 'pending', progress: 0, delay: 3000, duration: 1000, details: 'Reserve 2 hours on Friday for deep work on the backend architecture.' },
];

type AppState = 'idle' | 'dropping' | 'unpacking' | 'reviewing' | 'processing' | 'cleaning' | 'recorded';

export default function TaskUnpacker() {
  ...
  useEffect(() => {
    ...
    tasks.forEach((task, index) => {
      // Start processing after specific delay
        ...
        const progressInterval = setInterval(() => {
          setTasks(prev => {
            ...
            if (newProgress >= 100) {
              ...
              return prev.map(t => t.id === task.id ? { ...t, progress: 100, status: 'completed' } : t);
            }
            
            return prev.map(t => t.id === task.id ? { ...t, progress: newProgress } : t);
          });
        }, intervalTime);
  ...
  }, [state, tasks.length]); // Added tasks.length dependency to re-run if tasks change (e.g. deletion)

  // Act 4: Cleanup when all done
  useEffect(() => {
  ...
  const nextCard = () => {
    setCurrentCardIndex((prev) => (prev + 1) % tasks.length);
  };
          ...
          >
            ...
            {!sidebarMinimized && (
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {tasks.map(task => (
                  <div key={task.id} className="p-3 rounded-lg border border-stone-100 bg-stone-50 flex items-start gap-3">
                    <div className={`mt-0.5 p-1.5 rounded-md ${
                      ...
                      {task.status === 'completed' ? <CheckCircle size={14} /> :
                       task.status === 'processing' ? <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 2, ease: "linear" }}><Sparkles size={14} /></motion.div> :
                       <Clock size={14} />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium truncate">{task.title}</p>
                      <div className="flex justify-between items-center mt-1">
                        <span className="text-[10px] uppercase text-stone-400 font-semibold">{task.type}</span>
        ...
        <div className="relative w-full max-w-4xl h-[600px] flex items-center justify-center perspective-1000">
          ...
          <AnimatePresence>
            {(state === 'dropping' || state === 'unpacking') && (
              <motion.div
                ...
                animate={state === 'dropping' 
                  ? { y: 0, rotate: 0, opacity: 1 } 
                  : { scale: 0, opacity: 0 } // Disappear when unpacking
                }
                transition={state === 'dropping' 
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className={`w-4 h-4 rounded border flex items-center justify-center ${
                            selectedTaskIds.has(task.id) ? 'bg-blue-500 border-blue-500' : 'border-stone-300 bg-white'
                          }`}>
          ...
          <div className="relative w-full h-full flex items-center justify-center">
            {tasks.map((task, index) => {
              // Calculate random scatter positions for unpacking
              const angle = (index / tasks.length) * 360;
              const radius = 220; // Spread radius
              const x = Math.cos(angle * (Math.PI / 180)) * radius;
              const y = Math.sin(angle * (Math.PI / 180)) * radius * 0.6; // Flattened circle (ellipse)
              ...
              return (
                <motion.div
                  ...
                  animate={
                    isVisible 
                      ? isCleaning && !isCardStack
                        ? { x: 0, y: 200, scale: 0, opacity: 0, transition: { delay: index * 0.1 } } // Suck into box
                        : isCardStack
                          ? { scale: 0, opacity: 0 } // Hide original cards in stack mode (we render a separate stack)
                          : { 
                              scale: 1, 
                              x: x + (Math.random() * 40 - 20), 
                              y: y + (Math.random() * 40 - 20), 
                              opacity: isReviewing ? 0.4 : 1, // Fade out during review
                  ...
                  transition={{ 
                    ...
                    stiffness: 100, 
                    damping: 15, 
                    delay: isCleaning ? 0 : 0.2 + (index * 0.1) // Staggered entrance
                  }}
                ...
                >
                  ...
                  <div className="p-4 border-b border-stone-100 flex items-center justify-between bg-stone-50/50">
                    <div className="flex items-center gap-2">
                      {task.type === 'email' && <div className="p-1.5 bg-blue-100 text-blue-600 rounded-lg"><Mail size={14} /></div>}
                      {task.type === 'call' && <div className="p-1.5 bg-green-100 text-green-600 rounded-lg"><Phone size={14} /></div>}
                      {task.type === 'calendar' && <div className="p-1.5 bg-purple-100 text-purple-600 rounded-lg"><Calendar size={14} /></div>}
                      {task.type === 'research' && <div className="p-1.5 bg-amber-100 text-amber-600 rounded-lg"><ArrowRight size={14} /></div>}
                      <span className="text-xs font-semibold uppercase tracking-wider text-stone-500">{task.type}</span>
                    </div>
          ...
          {/* Act 4: Card Stack Overlay (Interactive) */}
          <AnimatePresence>
                ...
                <div className="relative w-96 h-[420px]">
                  ...
                  <div className="relative w-full h-full">
                    {tasks.map((task, index) => {
                      ...
                      const diff = (index - currentCardIndex + tasks.length) % tasks.length;
                      // We want to show: Current, Next, Prev visually
                      // Let's simplify: Show current on top, others behind
                      ...
                      // 0 = current
                      // 1 = next (behind right)
                      // -1 (or length-1) = prev (behind left)
                      ...
                      if (isCurrent) {
                        ...
                        zIndex = 10;
                        rotate = 0;
                      } else if (index === (currentCardIndex + 1) % tasks.length) {
                        x = 40;
                        scale = 0.9;
                        opacity = 0.6;
                        zIndex = 5;
                        rotate = 5;
                      } else if (index === (currentCardIndex - 1 + tasks.length) % tasks.length) {
                        x = -40;
                        scale = 0.9;
                        opacity = 0.6;
                        zIndex = 5;
                          ...
                          <div className="p-6 flex-1 flex flex-col">
                            <div className="flex justify-between items-start mb-4">
                              <div className={`p-2 rounded-lg ${
                                ...
                                task.type === 'calendar' ? 'bg-purple-100 text-purple-600' :
                            ...
                            <div className="mt-6 pt-4 border-t border-stone-100 flex justify-between items-center">
                              ...
                              <button 
                                ...
                                className={`flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-full transition-colors ${
                                  showWorkflow ? 'bg-emerald-100 text-emerald-700' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'
                                }`}
                              >
                                <Network size={14} />
                  ...
                  {/* Workflow Overlay for Card Stack */}
                  <AnimatePresence>
                    {showWorkflow && (
                      <motion.div
